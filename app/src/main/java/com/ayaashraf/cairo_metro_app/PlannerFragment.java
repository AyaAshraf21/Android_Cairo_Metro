...
//            exitSpinner.setText(sharedPreferences.getString(KEY_EXITSTATION, ""));
        ...
        adapter = new ArrayAdapter<>(getContext(), android.R.layout.simple_list_item_1, stations);
...
//            displayInputs();
        switchIcon.setOnClickListener(view1 -> {
        ...
        entrySpinner.setOnClickListener(v -> entrySpinner.showDropDown());

        entrySpinner.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus) {
        ...
        exitSpinner.setOnClickListener(v -> exitSpinner.showDropDown());

        exitSpinner.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus) {
