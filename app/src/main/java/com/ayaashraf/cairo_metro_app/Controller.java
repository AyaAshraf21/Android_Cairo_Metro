...
public class Controller {
    ...
    public static ArrayList<String> getSharedLine(String transitionStation1, String transitionStation2) {
    ...
    public static boolean isTransition(String transitionStation, ArrayList<String> path) {
    ...
    public static StringBuilder getDirection(ArrayList<String> path) {
        ...
        for (int i = 1; i < path.size() - 1; i++) {
            ...
            if (transtionStations.contains(currentStation)) {
                if(path.indexOf(currentStation)!=0 && path.indexOf(currentStation)!=path.size()-1) {
                    if (isTransition(currentStation, path)) {
                        int currentIndex = path.indexOf(currentStation);
                        if (currentIndex + 1 < path.size()) {
                            ArrayList<String> lineShared = getSharedLine(currentStation, path.get(currentIndex + 1));
                            if (!lineShared.isEmpty()) {
                                if (!currentStation.equals("Kit Kat") && !currentStation.equals("كيت كات")) {
                                    ...
                                    if (lineShared.indexOf(path.get(currentIndex + 1)) > lineShared.indexOf(currentStation)) {
                                ...
                                direction.append("invalid path 2").append(currentStation).append(".");
                            }
                        }
                    }
                }
